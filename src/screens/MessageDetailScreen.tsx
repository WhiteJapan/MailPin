...
import { HtmlMessageBody } from '../components/HtmlMessageBody';
  ...
  return (
    <main className="app-screen detail-screen">
      ...
      <article className="message-detail">
        ...
        {detail
          ? detail.bodyContentType === 'html'
            ? <HtmlMessageBody html={detail.body} title={`${summary.subject}の本文`} />
            : <pre className="message-body">{detail.body}</pre>
          : error
            ? <ErrorState message={error} onRetry={() => setAttempt((value) => value + 1)} />
            : <LoadingState label="本文を読み込んでいます" />}
